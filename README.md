...
On Windows, build a portable zip that includes an executable launcher, offline web assets, league config, generated empty state when needed, player data, and license notices:

```powershell
.\scripts\build_test_package.ps1
```

The build runs the projection-quality release gate first, so a degraded single-source or position-incomplete board cannot be shipped accidentally. Send `dist\DraftAssistant-test-package.zip` to your tester. They unzip it and double-click `Start Draft Assistant.bat`; no Python install is required.
