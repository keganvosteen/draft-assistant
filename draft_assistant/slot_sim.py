"""Compare rollout draft-score picks against ADP autodraft, by draft slot.

Answers "does the engine beat ADP from seat 7?" — it sweeps every seat in the
league and reports where the edge is largest. Drives `draft-assistant
simulate-strategy`.

Distinct from `strategy_sim.py`, which is the head-to-head benchmark harness
behind `scripts/run_draft_benchmark.py`. The two arrived on separate branches
under the same filename; this one was renamed on merge. They share no API.
"""
