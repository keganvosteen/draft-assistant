...
def _setup_wizard() -> Tuple[LeagueConfig, DraftState]:
    """Interactive league setup. Returns (config, state)."""
    _clear()
    _header("FANTASY DRAFT ASSISTANT — LEAGUE SETUP")
    ...
    provider = {
        "type": "local_json",
        "options": {"path": "data/projections.json"},
    }

    config = LeagueConfig(teams=teams, roster=roster, scoring=scoring, provider=provider)
    save_config(config)
...
def _show_board(tracker: DraftTracker, config: LeagueConfig, state: DraftState):
    ...
    ranked = suggest_players(config, avail, roster, top_n=10, total_picks=total_picks)
...
def run_interactive():
    ...
    if os.path.exists("league.config.yaml") and os.path.exists("data/projections.json"):
        print(f"\n  Found existing league config.")
        choice = _prompt("Start new league setup or continue? (new/continue)", "continue")
        if choice.lower().startswith("n"):
            config, state = _setup_wizard()
        else:
            config = load_config()
            state = load_state()
    else:
